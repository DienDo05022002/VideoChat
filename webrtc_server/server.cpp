    ...
    : QObject{parent}
{
    ...
    if(m_server->listen(QHostAddress::Any,4567)) {
        qDebug()<<"server start! listening on port 4567.....";
    }
    else {
...
{
    qDebug()<<"text received from client:"<<message;
    ...
    if(sendClient) {
        ...
        if(type=="signin") {
            ...
            qDebug()<<"user"<<username<<"signin. server assign an id:"<<m_count<<"for him(her)";
...
void Server::onDisconnection()
{
    ...
    if (websocketClient) {
        ...
        if(i!=-1)
        {
            qDebug() << "Client disconnected:" <<"username:"<< m_clients.value(i)->getUsername() << "peerid:"<< m_clients.value(i)->getPeerId();
            m_clients.remove(i);
